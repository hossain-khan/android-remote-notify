...
class PluginProvider : ContentProvider() {
    ...
    private fun insertNotification(values: ContentValues?): Uri? {
        ...
        val priority =
            values.getAsString(PluginContract.NotificationColumns.PRIORITY)
                ?: PluginContract.Priority.NORMAL
        val preferredMediums =
            values
                .getAsString(PluginContract.NotificationColumns.PREFERRED_MEDIUMS)
                ?.split(",")
                ?.map { it.trim() }
        ...
        return PluginContract.NOTIFICATIONS_URI
            .buildUpon()
    ...
    private suspend fun processNotificationRequest(request: PluginNotificationRequest) {
        try {
            ...
            val sendersToUse =
                if (request.preferredMediums != null) {
            ...
            Timber.i(
                "Plugin notification processed: ${request.requestId}, " +
    ...
    private fun getSendersForMediums(mediumNames: List<String>): List<NotificationSender> =
        notificationSenders.filter { sender ->
