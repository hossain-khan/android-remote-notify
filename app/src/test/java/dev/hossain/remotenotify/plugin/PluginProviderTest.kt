...
class PluginProviderTest {
    ...
    fun setup() {
        ...
        pluginProvider = spyk(PluginProvider()) {
    ...
    fun `queryConfig returns available mediums`() {
        ...
        // When
        val cursor = pluginProvider.query(
    ...
    @Test
    fun `insertNotification sends notification through configured mediums`() = runTest {
        ...
        every { context.packageManager } returns mockk {
            ...
            every { getApplicationInfo("com.example.test", 0) } returns mockk {
    ...
    @Test
    fun `insertNotification with preferred mediums filters senders`() = runTest {
        // Setup additional mock sender
        val mockTelegramSender = mockk<NotificationSender> {
